...
use libvips::{ops, ops::BandFormat, ops::ResizeOptions, VipsImage};
#[cfg(feature = "cli")]
...
impl<'f> ImgBackend<'f> {
    ...
    fn cairo_to_vips(img: &mut ImageSurface) -> Result<VipsImage> {
        ...
        VipsImage::new_from_memory(&data, w, h, 4, BandFormat::Uchar)
            .map_err(|_| Error::ImageConversionError("cairo", "vips"))
    }
    ...
    pub fn paint(
        ...
        if let Some(stroke) = stroke {
            let img = self.stroked(img, sx, sy, stroke)?;
    ...
    fn stroked(
        ...
        let img =
            ops::resize_with_opts(&img, sx, &opts).map_err(|e| Error::VipsError(e.to_string()))?;
